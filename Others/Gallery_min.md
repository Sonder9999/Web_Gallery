...
<!-- - 放大镜效果 -->
<!-- - 图片悬停原图效果 -->
  <!-- - 我还想做一个功能,就是当我在主页hover在图片上的时候(非图片详情页),能够显示原图的大小,如果原图过大,那就按照这个网页最多能显示的大小来放大,逻辑类似于放大镜,单独写一个js文件,并且注意加个开关能够控制是否开启这个功能 -->
  <!-- - 能够不更新gallery.js直接通过这一个js文件来完成吗,.现在gallery.js太长了,不好修改 -->
- gallery分文件
<!-- - 文件重新分类,例如每个website一个文件夹 -->
- 视频上传
- 修改瀑布流布局,改为向下加载,动态固定列宽
- 英文与中文切换(文件名,数据库)
- 我现在的数据库里存储的是原图,但是我的图片很多,一旦加载起来,就会导致用户很慢,能否让用户加载变快,例如加载的图片质量没有那么好,而且我也不用再单独上传一个低画质的文件,而且用户下载却能够下载原图的方法
